...
The ``sequence`` module extends the :doc:`typeid <mad_mod_types>` module with the ``is_sequence`` function, which returns ``true`` if its argument is a ``sequence`` object, ``false`` otherwise.
...
**siter**
   ...
   between the sequence attribute ``minlen`` and the ``minlen`` from the :doc:`constant <mad_mod_const>` module.
...
as arguments. These commands also save the iterator states in their ``mflw`` to allow the users to run them ``nstep`` by ``nstep``, see commands :doc:`survey <mad_cmd_survey>` 
and :doc:`track <mad_cmd_track>` for details.
